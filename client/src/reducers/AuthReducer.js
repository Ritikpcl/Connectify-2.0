const authReducer = (state = { authData: null, loading: false, error: false, updateLoading: false }, action) => {
  switch (action.type) {
    ...
    case "AUTH_SUCCESS":
      const { password, ...dataWithoutPassword } = action?.data;
      localStorage.setItem("profile", JSON.stringify(dataWithoutPassword));
      return { ...state, authData: dataWithoutPassword, loading: false, error: false };
